...
char* proxy(int vezes_while, int opcao, int primeira_requisicao)
{
	...
	do
    {
    	...
    	if (opcao == PROXY || opcao == INSPECAO || primeira_requisicao)
    	{
	        ...
	        if (tam_requisicao < 0) 
	        {
	            errx (EXIT_FAILURE, "Erro ao ler a requisicao");
	        }
		}

		if (opcao == INSPECAO)
		{
			printf ("[INSPECTOR] Requisicao recebida: %s\n", buffer_requisicao);

			printf("[INSPECTOR] Escreva a requisicao editada: ");
	    	...
	    	printf ("[INSPECTOR] Requisicao editada: %s\n", buffer_requisicao);
		}
        ...
        //se a conexao nao for http, uma mensagem e enviada para o usuario
		else
		{
...
/*!\brief requestOption funcao chamada pela funcao proxy, responsavel por filtrar requisicoes HTTPS
 *
 * Funcao da opcao request, retorna 1 se for conexao http e retorna 0 se for https
 */
int requestOption()
{
	int tam_requisicao;
...
/*!\brief replyOption funcao chamada pela funcao proxy, responsavel interceptar a resposta do servidor externo e retorna-la
 *
 * Funcao da opcao reply, retorna vetor de tamanho variavel com o resultado da requisicao
 */
		...
		do
		{
			...
			if(!(n<=0))
			{
				...
				if (opcao == DUMP || opcao == SPIDER)
				{
					...
					//mostrando uma mensagem de satisfacao para o cliente
...
/*!\brief dns funcao chamada pela funcao proxy, responsavel interceptar a resposta do servidor externo e retorna-la
 *
...
void dns(char *host, char *host_retorno)
{
	...
	//copiando endereço ip para o buffer da funcao principal
...
/*!\brief camposRequisicao funcao chamada pela funcao proxy, responsavel interceptar a resposta do servidor externo e retorna-la
 *
 * Funcao para separar a requisicao em host, diretorio, http
 */
void camposRequisicao(char *buffer_requisicao, char *host, char *requisicao, char *http)
{
	...
	//pegando a parte GET do cabeçalho da requisicao
	...
	//pegando a parte de requisicao e isolando somenete o diretorio que foi pedido
	...
	//verifica se tem mais de um diretorio na requisicao
	//pega outros diretorios da requisicao e concatena com o primeiro
	aux_req = strtok(NULL, "/");
	while (aux_req != NULL)
	{
		//concatena com a /
...
/*!\brief sendTextHeader funcao chamada pela funcao proxy, responsavel interceptar a resposta do servidor externo e retorna-la
 *
 * Funcao que manda a requisicao para o servidor externo
 */
...
/*!\brief esperandoRequisicao funcao chamada pela funcao proxy, responsavel interceptar a resposta do servidor externo e retorna-la
 *
...
/*!\brief inicioSocketProxy funcao chamada pela funcao proxy, responsavel interceptar a resposta do servidor externo e retorna-la
 *
...
int inicioSocketProxy (int porta) 
{
    ...
    // configuracao para fazer o proxy ouvir na porta definida
...
int pegaPath (char * path){
	...
	//checa pra ver se o primeiro caractere e /, pra ver se e http:// ou pra ver se e caracte
	if(path[4]==':'){
		b=path+7;
		p=b;
		fim=strchr((p+1), '/');
...
void pushArvore (char * modulo, int profundidade){
	...
	//o nivel 0 sempre existe. Bora ver se ja tem filho
	//se nao tiver filho, o filho zero e NULL
	if(pai->filho[0]==NULL){
		//aloca memória
		aux = (struct Spider*) malloc (sizeof(struct Spider));
		//não esquecer de novo do que era mais importante que e guardar a string no novo nó		
		strcpy(aux->modulo, modulo);
		//faz o pai apontar pra esse primogênito (apontar com carinho)
		pai->filho[0] = aux;
		
	//agora o caso de ja ter pelo menos um filho nesse nível	
	}else{
		i=0;
		aux=pai->filho[0];
		//se o nível não ta vazio, tem que ver se ja foi escrito
